...
module.exports = class {
    ...
    init() {
        this.pswObj = wx.getStorageSync('commonSecret') ? {
            step: 2,
            spassword: JSON.parse(wx.getStorageSync('commonSecret'))
        } : {};
        this.lastPoint = [];
        this.makeState();
    ...
    makeState() {
        if (this.pswObj.step == 2) {
            app.trigger('titleChanged', '约定一个Secret吧~');
    ...
    storePass(psw) {// touchend结束之后对密码和状态的处理
        if (this.pswObj.step == 1) {
            if (this.checkPass(this.pswObj.fpassword, psw)) {
                ...
                wx.setStorageSync('commonSecret', JSON.stringify(this.pswObj.spassword));
    ...
    updatePassword() {
        wx.removeStorageSync('commonSecret');
        wx.removeStorageSync('chooseType');
        this.pswObj = {};
        app.trigger('titleChanged', '绘制Secert');
        this.reset();
    }
    reset() {
        this.makeState();
